...
def get_default_api_key() -> Optional[str]:
    """Read a deployment-time fallback API key from secrets or the environment."""
    try:
...
with st.sidebar:
    st.header("配置")
    fallback_api_key = get_default_api_key()
    api_key = st.text_input(
        "DashScope API Key",
        value=fallback_api_key or "",
        type="password",
        help="仅用于当前 Streamlit 会话，不会写入项目文件。",
    ).strip()
    if api_key:
